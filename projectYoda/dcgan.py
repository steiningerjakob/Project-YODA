...
def load_lego_data():
    ...
    image_generator = data_generator.flow_from_directory(
        directory=os.path.join(INPUT_DIR, '.'), **flow_from_directory_params)
...
def get_image_batch(image_generator):
    ...
    assert img_batch.shape == (BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH,
                               N_CHANNELS), img_batch.shape
    return img_batch
...
def make_generator():
    ...
    # Upsample to 8x8
    generator.add(
        keras.layers.Conv2DTranspose(128, (4, 4),
                                     strides=(2, 2),
                                     padding='same',
                                     use_bias=False))
    generator.add(keras.layers.LeakyReLU(alpha=0.2))
    ...
    # Output 128x128
    generator.add(
        keras.layers.Conv2D(N_CHANNELS, (3, 3),
                            activation='tanh',
                            padding='same',
                            use_bias=False))
    return generator
...
def train_gan(gan, generator, discriminator, image_generator):
    ...
    for epoch in range(N_EPOCHS):
        ...
        if (epoch) % 100 == 0:
            ...
            print(
                f'Saving batch of generated images at adversarial step: {epoch}.'
            )
            ...
            img.save(
                os.path.join(OUTPUT_DIR,
                             'generated_lego_epoch_' + str(epoch) + '.png'))
            img = keras.preprocessing.image.array_to_img(real_images[0])
            img.save(
                os.path.join(OUTPUT_DIR,
                             'real_lego_epoch_' + str(epoch) + '.png'))
